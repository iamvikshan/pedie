...
mock.module('@lib/data/orders', () => ({
  ...
  createOrder: mock(() => Promise.resolve({ id: 'order-1', status: 'pending' })),
}))
...
describe('Account Pages', () => {
  ...
  describe('Account Layout', () => {
    test('redirects to signin if not authenticated', async () => {
      ...
      await expect(
        AccountLayout({ children: React.createElement('div', null, 'content') })
  ...
  describe('Wishlist Page', () => {
    ...
    test('shows sign in prompt when not authenticated', () => {
      mockUseAuth.mockReturnValue({ user: null, loading: false, profile: null })
