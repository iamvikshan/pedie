...
![Market](https://img.shields.io/badge/Market-Kenya-red)
...
```mermaid
mindmap
  root((Kenyan Electronics Dilemma))
    Premium Retailers
      High prices
      KES 65k+ for flagships
      Good warranty
      Established trust
    Grey Market
      Low prices
...
flowchart TB
    subgraph Revenue["💵 Revenue Streams"]
        R1[Device Sales]
        R2[Extended Warranty]
        R3[Trade-in Program]
        R4[Accessories]
    end

    subgraph Model["📦 Order Model"]
        M1[Preorder\n5% deposit] -->|Out of Stock| M2[Import & Deliver]
        M3[In-Stock] -->|Available| M4[Immediate Sale]
    end

    subgraph Margins["📊 Target Margins"]
        MG1["Phones: 15-20%"]
        MG2["Laptops: 18-25%"]
        MG3["Accessories: 12-15%"]
    end
```
...
```mermaid
pie title Target Customer Segments
    "Young Professionals (25-35)" : 40
    "Students (18-25)" : 25
    "Tech Enthusiasts" : 20
    "Upgraders (3-5yr old devices)" : 15
```
...
```mermaid
flowchart TB
    subgraph Sourcing["🛒 Sourcing (Tier 1 Suppliers)"]
        S1[Swappa\n$299-320/unit\nPeer-to-peer, strict QC]
        S2[Reebelo\n$325/unit\nProfessional refurbishing]
        S3[Back Market\n$340/unit\nLargest inventory]
    end

    subgraph Shipping["✈️ International Logistics"]
        T1[Order Placed] -->|2-3 days| T2[US Warehouse]
        T2 -->|2-3 days| T3[Aquantuo Consolidation]
        T3 -->|3-5 days| T4[Kenya Customs]
        T4 -->|1-2 days| T5[Nairobi Hub]
    end

    subgraph QA["🔍 Quality Assurance"]
        Q1[Visual Inspection]
        Q2[Power & Boot Test]
        Q3[Battery Health Check]
        Q4[All Features Test]
        Q5[Software Update]
    end
...
```mermaid
stateDiagram-v2
    [*] --> Inquiry: Customer asks for product
    Inquiry --> InStock: Available
    Inquiry --> Preorder: Out of stock

    Preorder --> DepositPaid: 5% deposit received
    DepositPaid --> Ordered: Order placed with supplier
    Ordered --> InTransit: Shipped via Aquantuo
    InTransit --> Arrived: At Nairobi hub
    Arrived --> QAPassed: Inspection passed

    InStock --> QAPassed
    QAPassed --> Delivered: Customer pays balance
    Delivered --> [*]

    Arrived --> QAFailed: Inspection failed
    QAFailed --> Refund: Full refund to customer
```
...
sequenceDiagram
    ...
    participant P as Pedie
    participant S as Swappa
    participant A as Aquantuo

    C->>P: Inquiry for iPhone 12 PM
    P->>C: Quote: KES 58,000 (5% deposit)
    C->>P: Pays KES 2,900 deposit
    P->>S: Orders iPhone ($299)
    S->>A: Ships to Aquantuo
    A->>P: Delivers to Nairobi (7-10 days)
    P->>P: QA Inspection ✓
    P->>C: Ready for pickup/delivery
    C->>P: Pays KES 55,100 balance
    P->>C: Delivers device

    Note over P: Total: KES 58,000
    Note over P: Costs: KES 46,810
    Note over P: Profit: KES 11,190
```
...
gantt
    ...
    section Soft Launch
    Network Outreach           :active, 2026-02, 2026-03
    First Test Orders          :active, 2026-02, 2026-03
    Gather Testimonials        :2026-03, 2026-04
    section Growth
    Google/Facebook Ads        :2026-04, 2026-06
    Tech Blogger Partnerships  :2026-04, 2026-06
    SKU Expansion              :2026-05, 2026-07
    section Scale
    Regional Expansion         :2026-07, 2026-09
    Loyalty Program            :2026-08, 2026-10
    Extended Warranty Launch   :2026-09, 2026-11
    Trade-in Program           :2026-10, 2026-12
```
