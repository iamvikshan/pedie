...
function executeSync() {
  ...
  try {
    ...
    if (config.syncApiKey) {
      const syncUrl = config.siteUrl + '/api/sync'
      const syncResponse = UrlFetchApp.fetch(syncUrl, {
        method: 'post',
      ...
      const syncCode = syncResponse.getResponseCode()
      const syncBody = syncResponse.getContentText()
      if (syncCode < 200 || syncCode >= 300) {
        Logger.log(
          'ERROR: Sync request failed — URL: ' +
            syncUrl +
            ', status: ' +
            syncCode +
            ', body: ' +
      ...
      Logger.log('Sync response: ' + syncBody)
    }

    // Step 2: Revalidate the ISR cache so pages reflect updated data
    const revalidateUrl = config.siteUrl + '/api/revalidate'
    const revalidateResponse = UrlFetchApp.fetch(revalidateUrl, {
      method: 'post',
      contentType: 'application/json',
      headers: {
    ...
    const revalidateCode = revalidateResponse.getResponseCode()
    const revalidateBody = revalidateResponse.getContentText()
    if (revalidateCode < 200 || revalidateCode >= 300) {
      Logger.log(
        'ERROR: Revalidation failed — URL: ' +
          revalidateUrl +
          ', status: ' +
          revalidateCode +
          ', body: ' +
    ...
    Logger.log('Revalidation response: ' + revalidateBody)
  } catch (error) {
